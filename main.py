...
def train(logger, cf, model, dataset):
    ...
    net = model.Net(cf, logger).cuda()#1
    ...
    dataloaders = {phase: DataLoader(dataset.DataCustom(cf, logger, phase=phase),
                                     ...
                                     shuffle={'train': True, 'val': False}[phase],
    ...
    for epoch in range(starting_epoch, cf.num_epochs + 1):
        ...
        for batchidx, dataset_outputs in enumerate(dataloaders['train']):
            tic_fw = time.time()
            # run model
            batch_outputs = net(dataset_outputs['inputs'], dataset_outputs['mask'],phase='train')
        ...
        with torch.no_grad():
            if cf.do_validation:
                ...
                for batchidx, dataset_outputs in tqdm(enumerate(dataloaders['val'])):
                    outputs = net(dataset_outputs['inputs'], dataset_outputs['mask'], phase='val')
...
def test(logger, cf, model, dataset):
    ...
    with torch.no_grad():
        for index, dataset_outputs in tqdm(enumerate(test_data_loader)):
            # print(dataset_outputs['inputs'].shape)
            result_dict = net(dataset_outputs['inputs'], dataset_outputs['mask'], phase='test')
...
def parse_args():
    ...
    parser.add_argument('--exp_source', type=str, default='./experiments/cesm_seg',)
    parser.add_argument('--exp_result', type=str, default='./result/cesm_seg',)
